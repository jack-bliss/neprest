...
var NepEndpoint_1 = __webpack_require__(52);
exports.NepEndpoint = NepEndpoint_1.NepEndpoint;
var NepHandler_1 = __webpack_require__(53);
exports.NepHandler = NepHandler_1.NepHandler;
var NepModule_1 = __webpack_require__(54);
exports.NepModule = NepModule_1.NepModule;
var NepMiddleware_1 = __webpack_require__(123);
exports.NepMiddleware = NepMiddleware_1.NepMiddleware;
...
exports.NepEndpoint = NepEndpoint;
...
Object.defineProperty(exports, "__esModule", { value: true });
var NepRouterService_1 = __webpack_require__(55);
function NepModule(config) {
    return function (ctor) {
        var routes = {};
        config.handlers.forEach(function (handler) {
            var _loop_1 = function (process_1) {
                if (handler.prototype.hasOwnProperty(process_1)) {
                    var mtd_1 = handler.prototype[process_1];
                    var path = '/' + (handler.hasAlias ? handler.alias : handler.table) + (mtd_1.path ? mtd_1.path : '');
                    if (mtd_1._isNepEndpoint) {
                        if (!routes.hasOwnProperty(mtd_1.method)) {
                            routes[mtd_1.method] = {};
                        }
                        routes[mtd_1.method][handler.name + '_' + process_1] = {
                            path: path,
                            params: Array.isArray(mtd_1.params) ? mtd_1.params : [],
                            controller: function (req, res, next) {
                                var request;
                                var query;
                                if (mtd_1.custom) {
                                    request = mtd_1.func(req.params, req.query, req.body);
                                    query = request.query;
                                }
                                else if (mtd_1.method === 'get') {
                                    request = mtd_1.func(req.params, req.query);
                                    query = 'SELECT ' +
                                    ...
                                    if (request.orderBy && request.orderBy.length) {
                                        var ordering = request.orderBy.replace('-', '');
                                ...
                                else if (mtd_1.method === 'post') {
                                    request = mtd_1.func(req.params, req.body);
                                    query = 'INSERT INTO ' +
                                        handler.table +
                                        ' (' + Object.keys(request.insert).join(', ') +
                                        ') VALUES (' + Object.keys(request.insert)
                                        .map(function (key) { return request.insert[key]; })
                                        .map(function (val) {
                                        if (typeof val === 'string') {
                                            return '\'' + val + '\'';
                                        }
                                ...
                                else if (mtd_1.method === 'delete') {
                                    request = mtd_1.func(req.params);
                                ...
                                pool.query(query).then(function (pool_response) {
                                    if (request.prep) {
                    ...
                    else if (mtd_1._isNepMiddleware) {
                        config.app.use(path, mtd_1.func);
                    }
                }
            };
            for (var process_1 in handler.prototype) {
                _loop_1(process_1);
            }
        });
        for (var middle in ctor.prototype) {
            if (ctor.prototype.hasOwnProperty(middle)) {
                config.app.use(config.path ? config.path : '', ctor.prototype[middle]);
            }
        }
        var ExpressRoutes = NepRouterService_1.NepRouterService(routes);
        config.app.use(config.path ? config.path : '', ExpressRoutes);
        config.app.get('/neproutes/' + config.path, function (req, res) {
            res.json(routes);
        });
        ctor.routes = routes;
    };
}
exports.NepModule = NepModule;
...
function NepRouterService(routes) {
    var router = express.Router();
    var url;
    var process;
    for (var method in routes) {
        if (routes.hasOwnProperty(method)) {
            for (var processName in routes[method]) {
...
exports.NepRouterService = NepRouterService;
...
module.exports = {
    ...
    'big5hkscs': {
        type: '_dbcs',
        table: function() { return __webpack_require__(33).concat(__webpack_require__(85)) },
...
exports.NepMiddleware = NepMiddleware;
